...
static inline auto check_shape(const NBArray& arr, std::string_view name, std::array<int, NDims> expected) {
    if (arr.ndim() != expected.size()) {
        throw std::runtime_error(
            fmt::format("Expected {} to have {} dimensions, but got {}", name, expected.size(), arr.ndim()));
    }
    for (int dim = 0; dim < expected.size(); ++dim) {
        if (arr.shape(dim) != expected[dim]) {
            throw std::runtime_error(fmt::format("Expected {} to have extent {} at dimension {}, but got {}",
                                                 name,
                                                 expected[dim],
                                                 dim,
                                                 arr.shape(dim)));
...
nb::dlpack::dtype to_dlpack_dtype(ETensorDType dtype) {
    switch (dtype) {
        case ETensorDType::FP32: return {static_cast<std::uint8_t>(nb::dlpack::dtype_code::Float), 32, 1};
        case ETensorDType::BF16: return {static_cast<std::uint8_t>(nb::dlpack::dtype_code::Bfloat), 16, 1};
        case ETensorDType::INT8: return {static_cast<std::uint8_t>(nb::dlpack::dtype_code::Int), 8, 1};
        case ETensorDType::BYTE: return {static_cast<std::uint8_t>(nb::dlpack::dtype_code::UInt), 8, 1};
        case ETensorDType::FP16: return {static_cast<std::uint8_t>(nb::dlpack::dtype_code::Float), 16, 1};
        case ETensorDType::INT32: return {static_cast<std::uint8_t>(nb::dlpack::dtype_code::Int), 32, 1};
        case ETensorDType::FP8_E4M3: return {static_cast<std::uint8_t>(nb::dlpack::dtype_code::UInt), 8, 1};  // ugh
    }
...
NB_MODULE(_surogate, m) {
    ...
    nb::class_<GPUUtilInfo>(
        ...
        .def_rw("throttle_reason",
                &GPUUtilInfo::throttle_reason,
        ...
        .def(
            ...
            [](const GPUUtilInfo& gpu_util) {
                return fmt::format(
                    ...
                    gpu_util.fan,
                    gpu_util.power,
                    gpu_util.power_limit,
                    gpu_util.temperature,
                    gpu_util.temp_slowdown,
                    gpu_util.gpu_utilization,
                    gpu_util.mem_utilization,
                    gpu_util.throttle_reason,
                    gpu_util.mem_free,
                    gpu_util.mem_total,
                    gpu_util.mem_reserved,
                    gpu_util.pcie_rx,
                    gpu_util.pcie_tx);
            },
            "Return a debug string representation.");

    nb::class_<GPUInfo>(m, "GPUInfo", "Information about a single GPU device.")
        ...
        .def(
            "__repr__",
            [](const GPUInfo& info) {
                return fmt::format(R"(GPUInfo(device_id={}, name='{}', total_memory={}, compute_capability={}.{}))",
                                   info.device_id,
                                   info.name,
                                   info.total_memory,
    ...
    nb::class_<SystemInfo>(m,
                           "SystemInfo",
                           "System information utility class.\n\n"
                           "Provides methods to query CUDA, NCCL, cuDNN versions and GPU information.")
        .def_static("get_cuda_driver_version",
                    &SystemInfo::get_cuda_driver_version,
                    "Get CUDA driver version.\n\n"
                    "Returns: Integer version (e.g., 12010 for CUDA 12.1).")
        .def_static("get_cuda_runtime_version",
                    &SystemInfo::get_cuda_runtime_version,
                    "Get CUDA runtime version.\n\n"
                    "Returns: Integer version (e.g., 12010 for CUDA 12.1).")
        .def_static("get_nccl_version",
                    &SystemInfo::get_nccl_version,
                    "Get NCCL version.\n\n"
                    "Returns: Integer version (e.g., 2180 for NCCL 2.18.0).")
        .def_static("get_cudnn_version",
                    &SystemInfo::get_cudnn_version,
                    "Get cuDNN version.\n\n"
                    "Returns: Integer version (e.g., 8906 for cuDNN 8.9.6).")
        .def_static("get_gpu_info",
                    &SystemInfo::get_gpu_info,
                    "Get information about all available GPUs.\n\n"
                    "Returns: List of GPUInfo objects.");

    nb::class_<PretrainedConfig>(m,
                                 "PretrainedConfig",
        ...
        .def_rw("num_key_value_heads",
                &PretrainedConfig::NumKeyValHeads,
        ...
        .def_rw("max_position_embeddings",
                &PretrainedConfig::MaxPositionEmbeddings,
        ...
        .def_prop_ro("head_size",
                     &PretrainedConfig::head_size,
        ...
        .def_prop_ro("model_name",
                     &PretrainedConfig::model_name,
                     "Canonical model name derived from the configuration.")
        .def_static(
            "from_pretrained",
            [](const std::string& name, const std::string& dtype_str) -> PretrainedConfig* {
    ...
    nb::class_<RuntimeOptions>(
        ...
        .def(
            "__init__",
            [](RuntimeOptions* t,
               const std::string& recompute,
               bool offload_residual,
               bool use_cuda_graphs,
               bool trigger_timing_events,
               bool cpu_training,
               bool offload_master,
               bool offload_quants,
               ...
               bool offload_grads,
               bool use_zero_copy,
               bool use_write_combined,
               bool shard_weights,
               bool persistent_quants,
               bool shard_gradients,
        ...
        .def_prop_rw(
            ...
            [](RuntimeOptions& self, const std::string& level) {
                self.Recompute = RuntimeOptions::parse_recompute_level(level);
            },
        ...
        .def_rw("long_context",
                ...
                "Enable tiled MLP execution for long context training (reduces memory at long seq_len).")
        ...
        .def_rw("offload_optimizer",
                &RuntimeOptions::OffloadOptimizer,
                "Offload optimizer state (momentum and variance buffers).")
        .def_rw("offload_grads", &RuntimeOptions::OffloadGrads, "Offload gradients.")
        .def_rw("offload_experts",
                &RuntimeOptions::OffloadExperts,
                "Offload MoE expert NF4 weights to CPU, stream on-demand.")
        .def_rw("selective_expert_dequant",
                &RuntimeOptions::SelectiveExpertDequant,
                "Only dequantize router-selected experts (reduces memory).")
        ...
        .def_rw("use_all_to_all_reduce",
                &RuntimeOptions::UseAllToAllReduce,
                "Use all-to-all reduce strategy when reducing gradients.")
        .def_rw("init_projections_to_zero",
                &RuntimeOptions::InitProjectionsToZero,
                "Initialize certain projections to zero (for experiments).")
        .def_rw("debug_memory_breakdown",
                &RuntimeOptions::DebugMemoryBreakdown,
                "Print detailed memory breakdown after model allocation.")
        .def_rw("use_fused_rope",
                &RuntimeOptions::UseFusedRope,
        ...
        .def_rw("jit_kernel_manifests",
                &RuntimeOptions::JitKernelManifests,
                "JIT kernel manifests: maps kernel name -> manifest JSON path.")
        .def_rw("router_aux_loss_coef",
                &RuntimeOptions::RouterAuxLossCoef,
                "MoE aux loss coefficient (-1 = use model config).")
        .def_rw("router_z_loss_coef",
                &RuntimeOptions::RouterZLossCoef,
                "MoE z-loss coefficient (-1 = use model config).")
        ...
        .def_prop_rw(
            "gradient_type",
            [](const RuntimeOptions* opt) { return opt->grad_dtype(); },
            [](RuntimeOptions* opt, const std::string& dtype_str) {
                opt->GradientType = opt_dtype_from_str(dtype_str);
            },
            "Optional override dtype for gradient computations (empty/None means default).")
        .def_prop_rw(
            "master_dtype",
            [](const RuntimeOptions* opt) { return cast_opt_dtype(opt->MasterDType); },
        ...
        .def_prop_rw(
            ...
            [](RuntimeOptions* opt, const std::string& backend_str) {
                opt->MatmulBackend = matmul_backend_from_str(backend_str);
            },
            "Matmul backend (auto, cublaslt, cutlass).")
        .def_prop_ro(
            "recipe_name",
            [](const RuntimeOptions* opt) { return std::string(opt->recipe_name()); },
            "Current training recipe name.")
        .def_prop_ro(
            "fp8_enabled",
            [](const RuntimeOptions* opt) { return opt->fp8_forward_enabled(); },
            "Whether FP8 forward pass is enabled.")
        .def_prop_ro("fp4_enabled", &RuntimeOptions::fp4_enabled, "Whether FP4 training is enabled.")
        .def(
            "set_recipe",
            [](RuntimeOptions* opt, const std::string& recipe_name) {
    ...
    nb::class_<LoRAAdapterConfig>(m,
        ...
        .def(
            ...
            [](LoRAAdapterConfig* t,
               ...
               bool use_rslora,
               bool train_router) {
                new (t) LoRAAdapterConfig{.Rank = rank,
                                          .Alpha = alpha,
                                          .Dropout = dropout,
                                          .TargetModules =
                                              std::set<std::string>(target_modules.begin(), target_modules.end()),
        ...
        .def(
            ...
            [](const LoRAAdapterConfig& cfg) {
                ...
                return fmt::format("LoRAAdapterConfig(rank={}, alpha={}, dropout={}, target_modules=[{}], dtype='{}', "
                                   "use_rslora={}, train_router={})",
                                   cfg.Rank,
                                   cfg.Alpha,
                                   cfg.Dropout,
                                   modules,
                                   dtype_to_str(cfg.DType),
                                   cfg.UseRSLoRA,
                                   cfg.TrainRouter);
            },
            "Return a debug string representation.");
    ...
    nb::enum_<modules::QLoRAQuantStrategy>(m, "QLoRAQuantStrategy", "Quantization strategy for QLoRA base weights.")
    ...
    nb::class_<modules::QLoRAConfig>(
        ...
        .def(
            "__init__",
            [](modules::QLoRAConfig* t,
               bool enabled,
               const std::string& strategy,
               int block_size,
               const std::string& base_dtype,
               const std::string& adapter_dtype) {
                ...
                if (strategy == "fp8" || strategy == "FP8") {
                    ...
                    throw std::runtime_error("Unknown QLoRA strategy: " + strategy +
                                             " (valid: none, fp8, nvfp4, bnb, prequant_fp8, prequant_nvfp4, "
                                             "prequant_mxfp4, prequant_bnb_nf4)");
                }
        ...
        .def_prop_rw(
            ...
            [](modules::QLoRAConfig* cfg, const std::string& dtype_str) {
                cfg->base_dtype = dtype_from_str(dtype_str);
            },
        ...
        .def_prop_rw(
            ...
            [](modules::QLoRAConfig* cfg, const std::string& dtype_str) {
                cfg->adapter_dtype = dtype_from_str(dtype_str);
            },
            "Dtype for LoRA adapter weights.")
        .def_rw("enable_four_over_six",
                &modules::QLoRAConfig::enable_four_over_six,
                "Enable Four Over Six (4/6) adaptive block scaling for NVFP4 quantization.")
        .def_prop_ro("is_quantized",
                     &modules::QLoRAConfig::is_quantized,
                     "Whether quantization is active (enabled and strategy != None).")
        .def_prop_ro("is_fp4", &modules::QLoRAConfig::is_fp4, "Whether using FP4 quantization.")
        .def_prop_ro("is_fp8", &modules::QLoRAConfig::is_fp8, "Whether using FP8 quantization.")
        .def_prop_ro("is_bnb", &modules::QLoRAConfig::is_bnb, "Whether using BitsAndBytes NF4 quantization.")
        .def_static(
            "fp8",
            [](int block_size) { return modules::QLoRAConfig::fp8(block_size); },
            nb::arg("block_size") = 128,
        ...
        .def_static(
            "bnb",
            [](int block_size, bool double_quant) { return modules::QLoRAConfig::bnb(block_size, double_quant); },
            nb::arg("block_size") = 64,
            nb::arg("double_quant") = true,
        ...
        .def_static(
            "prequant_bnb",
            [](bool double_quant) { return modules::QLoRAConfig::prequant_bnb(double_quant); },
        ...
        .def_rw("bnb_double_quant",
                &modules::QLoRAConfig::bnb_double_quant,
                "Enable double quantization for BnB (quantize absmax values to INT8).")
        .def_rw("bnb_double_quant_group_size",
                &modules::QLoRAConfig::bnb_double_quant_group_size,
                "Group size for double quantization (number of absmax values per group).")
        .def_rw("num_experts",
                &modules::QLoRAConfig::num_experts,
                "Number of experts for MoE models (0 = dense model, >0 = MoE model).")
        .def_rw("num_experts_per_tok",
                &modules::QLoRAConfig::num_experts_per_tok,
                "Number of experts selected per token (top-k routing).")
        .def_rw("moe_intermediate_size",
                &modules::QLoRAConfig::moe_intermediate_size,
                "Per-expert MLP intermediate size (0 = use regular intermediate_size).")
        .def_prop_ro("is_moe", &modules::QLoRAConfig::is_moe, "Whether this is an MoE model (num_experts > 0).")
        .def_prop_ro("is_prequantized",
                     &modules::QLoRAConfig::is_prequantized,
        ...
        .def(
            ...
            [](const modules::QLoRAConfig& cfg) {
                if (cfg.is_bnb()) {
                    return fmt::format("QLoRAConfig(enabled={}, strategy='{}', block_size={}, bnb_double_quant={}, "
                                       "adapter_dtype='{}')",
                                       cfg.enabled,
                                       modules::to_string(cfg.strategy),
                                       cfg.scale_config.block_size,
                                       cfg.bnb_double_quant,
                                       dtype_to_str(cfg.adapter_dtype));
                }
                return fmt::format(
                    "QLoRAConfig(enabled={}, strategy='{}', block_size={}, base_dtype='{}', adapter_dtype='{}')",
                    cfg.enabled,
                    modules::to_string(cfg.strategy),
                    cfg.scale_config.block_size,
                    dtype_to_str(cfg.base_dtype),
                    dtype_to_str(cfg.adapter_dtype));
            },
            "Return a debug string representation.");

    // Optimizer type enum
    nb::enum_<optimizers::OptimizerType>(
        m,
        "OptimizerType",
    ...
    nb::class_<optimizers::OptimizerConfig>(m,
                                            "OptimizerConfig",
        ...
        .def(
            ...
            [](optimizers::OptimizerConfig* t,
               ...
               float adamw_beta2,
               float adamw_epsilon,
               float normuon_momentum,
               float normuon_beta2,
               float normuon_lr,
               bool normuon_cautious_wd) {
                ...
                if (optimizer == "adamw") {
                    ...
                    throw std::runtime_error("Unknown optimizer type: " + optimizer +
                                             " (valid: adamw, adamw_8bit, normuon)");
                }
        ...
        .def_static("adamw",
                    &optimizers::OptimizerConfig::adamw,
                    nb::arg("lr") = 2e-4f,
                    nb::arg("beta1") = 0.9f,
                    nb::arg("beta2") = 0.999f,
        ...
        .def_static("adamw_8bit",
                    &optimizers::OptimizerConfig::adamw_8bit,
                    nb::arg("lr") = 2e-4f,
                    nb::arg("beta1") = 0.9f,
                    nb::arg("beta2") = 0.999f,
                    nb::arg("epsilon") = 1e-8f,
    ...
    nb::class_<MultiGPUPyTrainer>(m,
        ...
        .def(
            ...
            nb::arg("seq_len"),
            nb::arg("grad_accum"),
            nb::arg("memcpy_all_gather") = true,
            nb::arg("memcpy_send_recv") = true,
            nb::arg("lora_config") = std::nullopt,
            nb::arg("qlora_config") = std::nullopt,
        ...
        .def("set_adapter_path",
             &MultiGPUPyTrainer::set_adapter_path,
             nb::arg("path"),
        ...
        .def(
            "import_weights_from_external",
            [](MultiGPUPyTrainer& self, const std::string& safetensors_path, nb::list per_gpu_list) {
            ...
            nb::arg("safetensors_path"),
            nb::arg("per_gpu_weights"),
        ...
        .def("export_model",
             &MultiGPUPyTrainer::export_model,
             nb::arg("path"),
        ...
        .def_static(
            ...
            [](const std::string& name,
               ...
               std::optional<modules::QLoRAConfig> qlora_config) {
                ...
                options.ModelType = config.DType;
                auto trainer = new MultiGPUPyTrainer(ngpu,
                                                     config,
                                                     options,
                                                     batch_size,
                                                     seq_len,
        ...
        .def("init_weights", &MultiGPUPyTrainer::init_weights, "Initialize weights from scratch (random init).")
        .def("load_checkpoint",
        ...
        .def(
            ...
            [](MultiGPUPyTrainer* trainer, TokenArray inputs, TokenArray targets, TokenArray3 position_ids) {
                ...
                if (position_ids.shape(1) != batch_size * local_gpus || position_ids.shape(2) != seq_len) {
                    throw std::runtime_error(
                        "position_ids must have shape [planes, batch_size * local_gpus, seq_length]");
                }
                ...
                const std::size_t per_rank = static_cast<std::size_t>(planes) * static_cast<std::size_t>(batch_size) *
                    ...
                    for (int p = 0; p < planes; ++p) {
                        for (int b = 0; b < batch_size; ++b) {
                            const std::size_t src_base =
                                (static_cast<std::size_t>(p) * batch_size * local_gpus +
                                 static_cast<std::size_t>(r) * batch_size + static_cast<std::size_t>(b)) *
                                seq_len;
                            const std::size_t dst_base =
                                (static_cast<std::size_t>(p) * batch_size + static_cast<std::size_t>(b)) * seq_len;
        ...
        .def(
            ...
            [](MultiGPUPyTrainer* trainer,
               ...
               std::vector<FloatArray> deepstack_visual_embeds) {
                ...
                if (visual_embeds.shape(0) != static_cast<std::size_t>(batch_size * local_gpus * seq_len) ||
                    ...
                    throw std::runtime_error(
                        "visual_embeds must have shape [batch_size * local_gpus * seq_length, hidden]");
                }
            ...
            nb::arg("visual_pos_masks"),
            nb::arg("visual_embeds"),
            nb::arg("deepstack_visual_embeds") = std::vector<FloatArray>{},
        ...
        .def(
            "update_with_config",
            [](MultiGPUPyTrainer* trainer, const optimizers::OptimizerConfig& config, int step) {
        ...
        .def(
            ...
            [](MultiGPUPyTrainer* trainer,
               ...
               nb::object temperatures_obj) {
                ...
                trainer->step_with_custom_loss(input_ids.data(),
                                               targets.data(),
                                               per_token_grads.data(),
                                               position_ids_ptr,
                                               temperatures_ptr);
            },
            nb::arg("input_ids"),
        ...
        .def(
            ...
            [](MultiGPUPyTrainer* trainer,
               ...
               nb::object temperatures_obj) {
                ...
                auto logprobs = trainer->compute_logprobs(input_ids.data(),
                                                          targets.data(),
                                                          B,
                                                          T,
                                                          use_lora,
        ...
        .def_static(
            ...
            [](int ngpu,
               ...
               std::optional<modules::QLoRAConfig> qlora_config) {
                ...
                options.ModelType = config.DType;
                return new MultiGPUPyTrainer(ngpu,
                                             node_rank,
                                             num_nodes,
                                             nccl_id.c_str(),
                                             config,
    ...
    nb::class_<DataLoader>(m,
        ...
        .def(
            ...
            [](DataLoader* d,
               ...
               int rank = 0,
               int world_size = 1,
               unsigned long seed = 42) { new (d) DataLoader(file_list, chunk_size, rank, world_size, seed); },
            nb::arg("file_list"),
            nb::arg("chunk_size"),
            nb::arg("rank") = 0,
            nb::arg("world_size") = 1,
        ...
        .def(
            "load_batch",
            [](DataLoader* d, TokenArray inputs, TokenArray targets) {
        ...
        .def(
            "load_batch",
            [](DataLoader* d, TokenArray inputs, TokenArray targets, TokenArray position_ids) {
    ...
    nb::enum_<TrainingRunLogger::EVerbosity>(m, "LogVerbosity", "Logger verbosity level.")
    ...
    nb::class_<TrainingRunLogger>(
        ...
        .def(
            ...
            [](TrainingRunLogger* t,
               ...
               nb::object callback_obj,
               TrainingRunLogger::EVerbosity verbosity) {
            ...
            nb::arg("verbosity") = TrainingRunLogger::EVerbosity::DEFAULT,
            "Create a logger.\n\n"
            "Parameters:\n"
        ...
        .def(
            "log_cmd",
            [](TrainingRunLogger* logger, const std::vector<std::string>& args) {
        ...
        .def(
            "log_options",
            [](TrainingRunLogger* logger, const nb::dict& options) {
        ...
        .def("log_dataset",
             &TrainingRunLogger::log_dataset,
             nb::arg("train_loader"),
             nb::arg("eval_loader"),
             "Log dataset information.\n\n"
             "Parameters:\n- train_loader: Training DataLoader.\n- eval_loader: Evaluation DataLoader.")
        .def("log_step",
             nb::overload_cast<int, float, int, int, float, float, float>(&TrainingRunLogger::log_step),
             nb::arg("step"),
             nb::arg("epoch"),
             nb::arg("step_tokens"),
             nb::arg("duration_ms"),
        ...
        .def("log_eval",
             ...
             "- loss: Eval loss.")
        .def("log_moe_stats",
             &TrainingRunLogger::log_moe_stats,
             nb::arg("step"),
             nb::arg("aux_loss"),
             nb::arg("z_loss"),
        ...
        .def(
            "log_allocator",
            [](TrainingRunLogger* logger, const nb::dict& stats) {
        ...
        .def(
            "set_expected_time_per_token",
            [](TrainingRunLogger* logger, const MultiGPUPyTrainer* trainer) {
                ...
                auto ops = get_transformer_ops(
                    ...
                    options.sol_compute_dtype(is_qlora),
    ...
    // ---- Native Tokenizer ----
    nb::class_<tokenizer::Tokenizer>(m,
                                     "Tokenizer",
        ...
        .def("decode_single_token",
             &tokenizer::Tokenizer::decode_single_token,
             nb::arg("id"),
        ...
        .def("is_special_token",
             &tokenizer::Tokenizer::is_special_token,
             nb::arg("id"),
        ...
        .def(
            ...
            [](const tokenizer::Tokenizer& self, nb::list messages, bool add_generation_prompt) {
                ...
                for (auto item : messages) {
                    ...
                    msgs.push_back({nb::cast<std::string>(d["role"]), nb::cast<std::string>(d["content"])});
                }
                return self.apply_chat_template(msgs, add_generation_prompt);
            },
            nb::arg("messages"),
            nb::arg("add_generation_prompt") = false,
        ...
        .def(
            ...
            [](const tokenizer::Tokenizer& self, nb::list messages, const std::string& strategy_str) {
                ...
                for (auto item : messages) {
                    ...
                    msgs.push_back({nb::cast<std::string>(d["role"]), nb::cast<std::string>(content_obj)});
                }
