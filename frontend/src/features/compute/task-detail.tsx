...
export type ExtendedWorkload = WorkloadItem & { _task?: LocalTask };
...
export function statusForDot(s: string) {
  ...
  if (s === "provisioning" || s === "pending") return "deploying" as const;
        ...
        <div className="flex">
          ...
          <div className="flex-1 min-w-0 flex flex-col">
            {isTask ? (
              ...
              <div className="flex items-center justify-center h-full text-faint text-[11px] py-8">
                Logs available for local tasks only
              </div>
            )}
          </div>
        </div>
