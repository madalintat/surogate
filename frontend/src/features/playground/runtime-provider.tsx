...
function createDexieAdapter(
  ...
  return {
    ...
    async generateTitle(
      ...
      // Compare: wait until both threads are done
      if (pairId) {
        const paired = await db.threads
          .where("pairId")
          .equals(pairId)
        ...
        if (paired) {
          ...
          if (running[paired.id]) {
            setTimeout(() => {
              void createDexieAdapter(pairId).generateTitle(
                remoteId,
                messages,
              );
            }, 600);
      ...
      try {
        const title =
          (await generateTitleWithModel({ userText })) ||
...
function ActiveThreadSync({
  ...
  const mainThreadId = useAuiState(({ threads }) => threads.mainThreadId);
  const setActiveThreadId = usePlaygroundStore(
...
export function PlaygroundRuntimeProvider({
  ...
  return (
    <AssistantRuntimeProvider runtime={runtime}>
      <ActiveThreadSync enabled={!pairId} />
