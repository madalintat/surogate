...
When running `surogate vf-eval`, the evaluation dataset is used by default. If no `eval_dataset` is provided, evaluation falls back to the training dataset.
...
Verifiers includes several built-in stateful environment classes: `SandboxEnv` provides a containerized bash shell, and `PythonEnv` extends it with a persistent Python REPL. These handle sandbox lifecycle management automatically.
...
Environments are packaged as installable Python projects. The `surogate vf-init` command initializes a new environment project:

```bash
surogate vf-init my-env
```
...
These require additional dependencies installed via extras (e.g., `uv add 'verifiers[ta]'` for TextArena, `uv add 'verifiers[browser]'` for BrowserEnv, `uv add 'verifiers[openenv]'` for OpenEnvEnv). 
