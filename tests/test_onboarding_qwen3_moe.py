"""Onboarding test: Qwen3 MoE model.

Validates that the Surogate DSL forward pass matches HuggingFace reference
for a mini (truncated) Qwen3 MoE model. Compares per-layer hidden states and
final-norm outputs with tolerances.

Requirements:
    - GPU with enough VRAM for a small Qwen3 MoE model
    - HF weights: set QWEN3_MOE_MODEL_PATH env var or have Qwen/Qwen3-30B-A3B cached

Usage:
    pytest tests/test_onboarding_qwen3_moe.py -v --no-header
    QWEN3_MOE_MODEL_PATH=/path/to/Qwen3-30B-A3B pytest tests/test_onboarding_qwen3_moe.py -v
"""
...
def prepare_mini_model(snapshot_dir: Path) -> Path:
    """Create a truncated Qwen3 MoE model with NUM_LAYERS layers."""
...
def model_dir():
    ...
    if snapshot is None:
        pytest.skip(f"Qwen3 MoE weights not found. Set {ENV_VAR} or cache {MODEL_ID}")
...
class TestQwen3MoEOnboarding:
    ...
    def test_summary(self, forward_results):
        ...
        print("\n--- Qwen3 MoE Forward Compare (Surogate vs HF) ---")
